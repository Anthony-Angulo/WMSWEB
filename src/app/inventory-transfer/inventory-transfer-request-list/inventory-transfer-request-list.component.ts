...
export class InventoryTransferRequestListComponent implements OnInit, AfterViewInit, OnDestroy {

  @ViewChild(DataTableDirective) datatableElement: DataTableDirective;
  dtOptions: DataTables.Settings = {};
  dtTrigger: Subject<InventoryTransferRequestListComponent> = new Subject();
  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      rowCallback: (row: Node, data: any | object, index: number) => {
        ...
        $('td', row).off('click');
        $('td', row).on('click', () => {
          this.router.navigate(['/InventoryTransfer/Request/' + data.DocEntry]);
        });
        return row;
      },
      dom: 'ltipr',
